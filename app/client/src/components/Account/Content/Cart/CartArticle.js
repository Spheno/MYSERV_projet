...
export default class CartArticle extends React.Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      this.props.updateTotal(productRes.price);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  addToFavs = async data => {
    try {
      console.log("data addToFavs", data);
      const response = await userAPI.addToFavs(data);
      if (response) {
      ...
      <article className="overflow-hidden rounded-lg shadow-lg">
        ...
        <footer className="relative flex items-center p-2 leading-none md:p-4">
          ...
          <div className="absolute right-0 flex justify-end text-2xl">
            ...
            <button
              className="px-1 m-1 text-gray-700 no-underline hover:text-red-600"
              onClick={() =>
                this.addToFavs({
